---
title: "final 키워드를 사용하면, 어떤 이점이 있나요?"
tags: [Java]
status: published
---
...
list.add("hello");      // 내부 조작은 가능 O
list = new ArrayList<>(); // 참조 변경은 불가 X
```
...
class Child extends Parent {
    void validate() { }    // 컴파일 에러 X — final 메서드 오버라이딩 불가
    void process() { }     // 정상 O
}
```
...
class MyString extends String { }  // 컴파일 에러 X — 상속 불가
```
...
void transfer(final int amount) {
    amount = amount - fee;     // 컴파일 에러 X — 실수 사전 차단
...
```java
final List<String> list = new ArrayList<>();
list.add("A");       // 가능 O — 내부 상태 변경
list.add("B");       // 가능 O
list.clear();        // 가능 O

// 진짜 불변 리스트를 원하면
final List<String> list = List.of("A", "B");   // Unmodifiable
list.add("C");       // UnsupportedOperationException X
```
