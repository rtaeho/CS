...
→ 서버가 중간에 바뀌면? 새 서버는 이전 대화를 모름 → 장애 X

[Stateless — 서버가 기억하지 않음]
요청 1: "사과 2개 주세요"               → 서버A: 처리
요청 2: "사과 2개 + 배 1개 주세요"       → 서버B: 처리 가능 O
요청 3: "사과 2개 + 배 1개 결제할게요"   → 서버C: 처리 가능 O
...
Stateless 서버:
클라이언트 → 로드밸런서 → 서버A, B, C 아무 데나 가능 O
```
...
|**세션**|서버 (메모리/Redis)|X (서버가 상태 보유)|
|**JWT**|클라이언트 (토큰 자체에 정보 포함)|O (서버 저장 없음)|
