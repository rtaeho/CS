...
|Method|멱등|안전|이유|
|---|---|---|---|
|**GET**|O|O|조회만 수행, 서버 상태 변경 없음|
|**HEAD**|O|O|GET과 동일하나 본문 없음|
|**OPTIONS**|O|O|서버 정보 확인만|
|**PUT**|O|X|같은 데이터로 전체 교체 → 결과 항상 동일|
|**DELETE**|O|X|이미 삭제된 리소스 재삭제해도 상태 동일|
|**POST**|X|X|호출할 때마다 새 리소스 생성|
|**PATCH**|X|X|구현에 따라 결과가 달라질 수 있음|
...
→ 같은 데이터로 덮어쓰기 → 결과 동일 → 문제 없음 O

DELETE /orders/1 → (응답 유실) → 재시도
→ 이미 삭제됨 → 상태 동일 → 문제 없음 O

[멱등하지 않은 요청 — 재시도 위험]
POST /orders {상품: A, 수량: 1} → (응답 유실) → 재시도
→ 주문이 2건 생성됨 → 중복 주문 발생 X
...
HTTP/1.1 200 OK
{"orderId": 100, "status": "CREATED"}  ← 새로 생성하지 않고 이전 결과 반환 O
```
