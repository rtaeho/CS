---
title: "Stateless의 의미를 살펴보면, 세션은 적절하지 않은 인증 방법 아닌가요?"
tags: [HTTP, 세션, 인증]
status: published
---
...
세션 방식:
관리자 → session.invalidate("abc123") → 즉시 로그아웃 O
...
|시나리오|세션|JWT|
|---|---|---|
|**강제 로그아웃**|세션 삭제로 즉시 가능 O|불가능 (블랙리스트 필요)|
|**동시 접속 제한**|세션 수 카운팅으로 간단 O|서버 저장 없이는 불가능|
|**권한 즉시 변경**|세션 데이터 수정으로 즉시 반영 O|토큰 재발급 전까지 이전 권한 유지|
|**토큰/세션 탈취 대응**|세션 무효화로 즉시 차단 O|만료까지 차단 불가|
