---
title: "규모가 커져 서버가 여러 개가 된다면, 세션을 어떻게 관리할 수 있을까요?"
tags: [HTTP, 세션]
status: published
---
...
클라이언트 → 로드밸런서 → 서버B: 마이페이지 → 세션 없음 → 로그인 풀림 X
...
모든 서버가 모든 세션을 보유
→ 어디로 요청이 가도 세션 조회 가능 O
→ 서버 N대 → 세션 1개당 N-1번 복제 필요
```
