...
| **PUT**               | 리소스 전체 교체                  | 있음       | O   | X   |
| **PATCH**             | 리소스 부분 수정                  | 있음       | X   | X   |
| **DELETE**            | 리소스 삭제                     | 없음 또는 있음 | O   | X   |
| **HEAD**              | GET과 동일하지만 응답 본문 없이 헤더만 반환 | 없음       | O   | O   |
| **[[OPTIONS(http)]]** | 서버가 지원하는 메서드 확인            | 없음       | O   | O   |
...
```
GET    → 조회만 함 → 서버 상태 변경 없음 → 안전 O
POST   → 새 리소스 생성 → 서버 상태 변경 → 안전 X
DELETE → 리소스 삭제 → 서버 상태 변경 → 안전 X
```
