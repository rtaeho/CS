...
#### 컬렉션 Fetch Join과 페이징을 함께 쓸 때 주의점

`~ToMany` 관계에서 Fetch Join과 [[페이징 쿼리]](`Pageable`)를 함께 사용하면 다음과 같은 경고가 발생합니다.

```
firstResult/maxResults specified with collection fetch; applying in memory
```

컬렉션을 Fetch Join하면 Member 수만큼 결과 행이 늘어나므로(카티션 곱) DB 레벨에서 정상적인 `LIMIT`/`OFFSET` 페이징을 적용할 수 없습니다. 그래서 하이버네이트는 조인 결과 전체를 메모리에 올린 뒤 애플리케이션 레벨에서 페이징을 수행하는데, 대상 데이터가 많으면 이 과정에서 OutOfMemoryError가 발생할 수 있습니다.

가장 안전한 해결책은 컬렉션을 Fetch Join하지 않고 아래 3번의 Batch Size로 N+1을 해결하는 것입니다. Fetch Join은 `~ToOne` 관계이거나 페이징이 필요 없는 경우에만 사용하는 것이 안전합니다.
