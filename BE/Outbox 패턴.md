...
DB 쓰기와 외부 메시지 발행이 하나의 작업처럼 보이더라도, 실제로는 서로 다른 시스템에 대한 이중 쓰기입니다.
...
반대로 메시지는 발행됐지만 DB 트랜잭션 커밋이 실패하면, 외부 시스템은 존재하지 않는 주문 이벤트를 처리할 수 있습니다. 이런 문제를 줄이기 위해 비즈니스 데이터 저장과 이벤트 저장을 같은 DB 트랜잭션에 묶는 방식을 사용합니다.
...
이처럼 서비스 로직 실행과 이벤트 발행 준비를 원자적으로 다루는 방식을 트랜잭셔널 메시징이라고 볼 수 있습니다.
...
- Outbox 레코드는 발행 상태와 재시도 횟수, 마지막 에러를 함께 관리하는 편이 좋습니다.
- 발행은 at-least-once가 되기 쉬우므로 Consumer는 [[멱등성]]을 보장해야 합니다.
- Polling 방식은 간단하지만 지연이 생길 수 있고, [[CDC]] 방식은 실시간성이 좋지만 인프라 복잡도가 올라갑니다.
