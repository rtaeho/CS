...
```
장점
O 쓰기 응답 속도 매우 빠름 (캐시만 저장 후 즉시 응답)
O DB에 배치로 모아서 저장 → DB 부하 감소
O 쓰기가 폭발적으로 많은 상황에 유리

단점
X 캐시 장애 시 DB에 저장 안 된 데이터 유실 위험
X 캐시와 DB 일시적 불일치
X 구현 복잡도 높음
```
...
|상황|적합 여부|
|---|---|
|쓰기가 매우 잦은 서비스|O (게임 점수, 좋아요 수)|
|실시간 로그/이벤트 수집|O|
|금융/결제 데이터|X (데이터 유실 치명적)|
|정합성이 중요한 도메인|X|
