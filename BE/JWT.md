...
- Payload는 Base64URL로 인코딩될 뿐 암호화가 아니므로 민감 정보를 담지 않습니다.
- 비밀 키나 개인 키가 유출되면 토큰 위조 위험이 커지므로 안전하게 관리해야 합니다.
- Access Token은 짧게 두고 Refresh Token Rotation 같은 전략으로 탈취 피해를 줄입니다.
- `none` 알고리즘이나 약한 알고리즘을 허용하지 않도록 검증 라이브러리 설정을 확인합니다.
- 서버에서 즉시 무효화하기 어렵기 때문에 블랙리스트나 토큰 버전 전략이 필요할 수 있습니다.
