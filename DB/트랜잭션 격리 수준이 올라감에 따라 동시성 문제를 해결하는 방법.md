---
title: "트랜잭션 격리 수준이 올라감에 따라 동시성 문제를 해결하는 방법"
tags: [격리수준, MVCC, 동시성]
status: published
---
...
트랜잭션B: SELECT price → 50000 읽음 ← Dirty Read X
...
트랜잭션B 시작
  ...
  SELECT → 스냅샷② 생성 → 39000 ← Non-Repeatable Read X

[REPEATABLE READ — 스냅샷 고정]
트랜잭션B 시작
  SELECT → 스냅샷① 생성 → 45000
  (트랜잭션A: UPDATE 39000 + COMMIT)
  SELECT → 스냅샷① 재사용 → 45000 O Non-Repeatable Read 방지
...
READ COMMITTED:
  ...
  → (10000, 20000) 사이에 INSERT 가능 → 팬텀 발생

REPEATABLE READ (InnoDB):
  → 레코드 락 + 갭 락 = 넥스트 키 락
  → (10000, +∞) 전체에 갭 락
  → 이 범위에 INSERT 불가 → 팬텀 방지 O
```
