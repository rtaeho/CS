...
트랜잭션B: SELECT 상품 가격 → 이전 버전을 읽음 → 대기 없음 O
...
T2: 트랜잭션A: SELECT price → 스냅샷 생성
    ...
    → version(trx_id: 101, price: 45,000) 확인 → 커밋됨 → 이 값을 읽음 O
...
→ Non-Repeatable Read 방지 O
```

|격리 수준|스냅샷 생성 시점|같은 트랜잭션 내 일관성|
|---|---|---|
|**READ COMMITTED**|매 SELECT마다|보장하지 않음|
|**REPEATABLE READ**|트랜잭션 첫 SELECT 시|보장 O|

## MVCC가 해결하는 것과 해결하지 못하는 것

```
[MVCC가 해결하는 것]
O 읽기-쓰기 동시 수행 (서로 차단하지 않음)
O 더티 리드 방지 (커밋된 버전만 읽음)
O 반복 읽기 보장 (REPEATABLE READ에서)
O 일관된 읽기 (Consistent Read)

[MVCC가 해결하지 못하는 것]
X 쓰기-쓰기 충돌 → 여전히 락 필요
X 갱신 분실 (Lost Update) → 비관적/낙관적 락으로 해결
```
...
트랜잭션B: UPDATE 재고 = 9 + COMMIT ← 갱신 분실 X
...
읽기 + 읽기: MVCC로 둘 다 대기 없이 처리 O
읽기 + 쓰기: MVCC로 읽기는 이전 버전 조회, 쓰기는 최신 데이터 수정 O
쓰기 + 쓰기: 락으로 순차 처리 (하나가 대기) O
```
