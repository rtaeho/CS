...
```
[장점]
O 빠른 초기 로딩 (FCP) — 콘텐츠가 즉시 보임
O SEO 유리 — 크롤러가 완성된 HTML을 인식
O 소셜 미디어 공유 — OG 태그가 서버에서 설정됨
O 저사양 기기 지원 — 서버가 렌더링 부담

[단점]
X 서버 부하 — 매 요청마다 렌더링
X TTFB 증가 — 서버 렌더링 시간만큼 응답 지연
X Hydration 비용 — JS 로드 전까지 인터랙션 불가
X 서버 인프라 필요 — CDN만으로 배포 불가
```
