...
→ 페이지 깜빡임 없음 → 네이티브 앱 같은 부드러운 전환 O
→ 필요한 데이터만 API로 가져옴
```
...
```
[장점]
O 부드러운 페이지 전환 — SPA로 네이티브 앱 같은 UX
O 서버 부하 낮음 — 정적 파일만 배포, 서버 렌더링 없음
O 배포 간단 — CDN / S3에 정적 파일 업로드로 끝
O 프론트-백 완전 분리 — API만 있으면 됨
O 인터랙션 풍부 — 복잡한 UI 상호작용에 유리

[단점]
X 초기 로딩 느림 — JS 번들 다운로드 + 실행 + API 호출
X SEO 불리 — 크롤러가 빈 HTML만 봄
X 소셜 미디어 공유 — OG 태그가 빈 HTML에 없으면 미리보기 안 됨
X JS 번들 크기 — 앱이 커지면 번들도 커짐 → 초기 로딩 악화
X 저사양 기기 — 브라우저가 렌더링 부담
```
...
|서비스 유형|권장 방식|이유|
|---|---|---|
|**관리자 대시보드**|CSR O|SEO 불필요, 복잡한 인터랙션|
|**사내 툴**|CSR O|SEO 불필요, 빠른 개발|
|**웹 앱 (노션, 피그마 등)**|CSR O|앱에 가까운 UX 필요|
