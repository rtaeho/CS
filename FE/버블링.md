...
child.addEventListener("click", () => {
    console.log("child 2");     // 실행됨 O
});

parent.addEventListener("click", () => {
    console.log("parent");      // 실행 안 됨 X
});
```
...
// focusin은 버블링 됨
parent.addEventListener("focusin", handler); // 자식 focus 시 잡힘 O
```
