...
|Method|멱등|이유|
|---|---|---|
|**GET**|O|조회만 하므로 서버 상태 변경 없음|
|**PUT**|O|같은 데이터로 덮어쓰기 → 몇 번 해도 결과 동일|
|**DELETE**|O|이미 삭제된 리소스를 다시 삭제해도 상태 동일|
|**HEAD**|O|GET과 동일하나 본문 없음 → 서버 상태 변경 없음|
|**OPTIONS**|O|서버 정보 확인만 → 변경 없음|
|**POST**|X|호출할 때마다 새 리소스가 생성됨|
|**PATCH**|X|구현에 따라 결과가 달라질 수 있음|
...
→ 서버: 이미 처리된 키 → 이전 결과 반환 → 중복 생성 방지 O
```
