...
|**GET**|O||
|**HEAD**|O||
|**POST**|O (Content-Type 조건 추가)||
|PUT||O Preflight 발생|
|DELETE||O Preflight 발생|
|PATCH||O Preflight 발생|
...
3. 브라우저: 응답의 CORS 헤더 확인
   - Access-Control-Allow-Origin이 일치하면 → JS에 응답 전달 O
   - 일치하지 않으면 → JS에 응답 차단 X
...
1. Content-Type: application/json → Preflight O
2. Authorization: Bearer JWT     → Preflight O
3. PUT, DELETE 메서드 사용        → Preflight O
4. 커스텀 헤더 (X-Request-Id 등) → Preflight O
...
|실무 상황|단순 요청 여부|이유|
|---|---|---|
|GET /api/products (인증 없음)|O|기본 헤더만 사용|
|GET /api/products (JWT)|X|Authorization 헤더|
|POST /api/products (JSON)|X|application/json|
|PUT /api/products/1|X|PUT 메서드|
|DELETE /api/products/1|X|DELETE 메서드|
|POST /login (폼 전송)|O|form-urlencoded|
|이미지, CSS, JS 로드|O|단순 GET|
